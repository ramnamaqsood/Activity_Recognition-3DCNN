...
json_file = open('/home/hassan/usb_drive/keras_models/Activity_Accidents_32m_04032020_arch_ucf_accidents_32mp.json', 'r')
...
model.load_weights('/home/hassan/usb_drive/keras_models/Activity_Accidents_32m_04032020_weights_ucf_accidents_32mp.h5')
...
def get_class_names():
    class_names = ['Road Accident', 'Normal Situation']
    #with open(path) as f:
    #    for row in f:
...
def classify_video(video_path, output_path):
    ...
    # constructing video frames batch for Conv3D input:
        ...
        for frame in frames_copy:
            ...
            cv.putText(output, text, (20, 20), cv.FONT_HERSHEY_DUPLEX, 0.50, (0, 255, 0), 2)
            text = "Accident Probability: {0:0.2f}".format(pred_npy[0][0])
            cv.putText(output, text, (20, 40), cv.FONT_HERSHEY_SIMPLEX, 0.50, (0, 255, 0), 2)
            text = "Normal Probability: {0:0.2f}".format(pred_npy[0][1])
            cv.putText(output, text, (20, 60), cv.FONT_HERSHEY_SIMPLEX, 0.50, (0, 255, 0), 2)
